...
| 日期 | 功能域 | 用户价值 | 变更摘要 |
| --- | --- | --- | --- |
| 2026-05-09 | Guided Chrome Setup | Chrome Web Store 审核期的安装路径不再依赖 Chrome Stable 会拒绝的自分发 CRX；用户可以下载 release zip 后按 Chrome 官方 unpacked extension 流程完成安装。 | 发布 `0.1.11` patch 版本，`setup release/offline` 改为下载 release zip、展开到 Application Support、写入稳定 unpacked extension key、注册对应 native host allowed origin，并打开 `chrome://extensions/` 引导 Load unpacked；GitHub Release 仍保留 CRX 作为制品归档。 |
