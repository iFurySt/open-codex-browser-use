...
| 日期 | 功能域 | 用户价值 | 变更摘要 |
| --- | --- | --- | --- |
| 2026-05-09 | JS SDK High-Level Browser API | JS SDK 用户可以保留底层 JSON-RPC/CDP 原子调用，同时用 `browser.newTab()`、`tab.goto()`、`tab.playwright.waitForLoadState()` 和 `tab.playwright.domSnapshot()` 编排常见浏览器自动化流程。 | 新增 `connectOpenBrowserUse`、`OpenBrowserUseBrowser`、`OpenBrowserUseTab` 和 Playwright-like tab helper，并用 fake native socket 测试覆盖 navigation event wait、load-state fast path 和 DOM snapshot。 |
